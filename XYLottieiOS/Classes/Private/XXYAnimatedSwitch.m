...
@implementation XXYAnimatedSwitch {
...
+ (instancetype _Nonnull)switchNamed:(NSString * _Nonnull)toggleName {
  return [XXYAnimatedSwitch switchNamed:toggleName inBundle:[NSBundle mainBundle]];
}
...
+ (instancetype _Nonnull)switchNamed:(NSString * _Nonnull)toggleName inBundle:(NSBundle * _Nonnull)bundle {
  XXYComposition *composition = [XXYComposition animationNamed:toggleName inBundle:bundle];
  XXYAnimatedSwitch *animatedControl = [[XXYAnimatedSwitch alloc] initWithFrame:CGRectZero];
  if (composition) {
...
- (void)setAnimationComp:(XXYComposition *)animationComp {
...
- (BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
  ...
  if (XXY_PointDistanceFromPoint(_touchTrackingStart, location) > self.bounds.size.width * 0.25) {
  ...
  if (_on) {
    ...
    if (diff <= 0) {
      ...
      self.animationView.animationProgress = XXY_RemapValue(diff, 0, self.bounds.size.width, _offStartProgress, _offEndProgress);
    ...
    if (diff <= 0) {
      ...
      diff = MAX(MIN(self.bounds.size.width, diff), 0);
      self.animationView.animationProgress = XXY_RemapValue(diff, 0, self.bounds.size.width, _onStartProgress, _onEndProgress);
