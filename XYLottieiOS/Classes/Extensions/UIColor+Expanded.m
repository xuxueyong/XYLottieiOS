...
// Static cache of looked up color names. Used with +XXY_colorWithName:
...
- (NSArray *)XXY_arrayFromRGBAComponents {
	NSAssert(self.canProvideRGBComponents, @"Must be an RGB color to use -XXY_arrayFromRGBAComponents");
  
	CGFloat r,g,b,a;
	if (![self XXY_red:&r green:&g blue:&b alpha:&a]) return nil;
...
- (BOOL)XXY_red:(CGFloat *)red green:(CGFloat *)green blue:(CGFloat *)blue alpha:(CGFloat *)alpha {
...
- (UInt32)rgbHex {
	...
	CGFloat r,g,b,a;
	if (![self XXY_red:&r green:&g blue:&b alpha:&a]) return 0;
...
- (UIColor *)XXY_colorByMultiplyingBy:(CGFloat)f {
	return [self XXY_colorByMultiplyingByRed:f green:f blue:f alpha:1.0f];
}

- (UIColor *)XXY_colorByAdding:(CGFloat)f {
	return [self XXY_colorByMultiplyingByRed:f green:f blue:f alpha:0.0f];
}

- (UIColor *)XXY_colorByLighteningTo:(CGFloat)f {
	return [self XXY_colorByLighteningToRed:f green:f blue:f alpha:0.0f];
}

- (UIColor *)XXY_colorByDarkeningTo:(CGFloat)f {
	return [self XXY_colorByDarkeningToRed:f green:f blue:f alpha:1.0f];
}
...
- (UIColor *)XXY_colorByMultiplyingByColor:(UIColor *)color {
	...
	return [self XXY_colorByMultiplyingByRed:r green:g blue:b alpha:1.0f];
}
...
- (UIColor *)XXY_colorByAddingColor:(UIColor *)color {
	...
	return [self XXY_colorByAddingRed:r green:g blue:b alpha:0.0f];
}
...
- (UIColor *)XXY_colorByLighteningToColor:(UIColor *)color {
	...
	return [self XXY_colorByLighteningToRed:r green:g blue:b alpha:0.0f];
}
...
- (UIColor *)XXY_colorByDarkeningToColor:(UIColor *)color {
	...
	return [self XXY_colorByDarkeningToRed:r green:g blue:b alpha:1.0f];
}

#pragma mark String utilities

- (NSString *)XXY_stringFromColor {
	NSAssert(self.canProvideRGBComponents, @"Must be an RGB color to use -XXY_stringFromColor");
	NSString *result;
	switch (self.colorSpaceModel) {
		case kCGColorSpaceModelRGB:
...
// Returns a UIColor by scanning the string for a hex number and passing that to +[UIColor XXY_colorWithRGBHex:]
...
+ (UIColor *)XXY_colorWithHexString:(NSString *)stringToConvert {
  NSString *strippedString = [stringToConvert stringByReplacingOccurrencesOfString:@"#" withString:@""];
	...
	return [UIColor XXY_colorWithRGBHex:hexNum];
}
...
+ (UIColor *)searchForColorByName:(NSString *)cssColorName {
	...
	if (found) {
		...
		if (sscanf(after, "%x", &hex) == 1) {
			result = [self XXY_colorWithRGBHex:hex];
		}
	}
