...
@implementation XXYCompositionContainer {
...
- (instancetype)initWithModel:(XXYLayer *)layer
               ...
               withLayerGroup:(XXYLayerGroup *)childLayerGroup
              withAssestGroup:(XXYAssetGroup *)assetGroup {
...
- (void)initializeWithChildGroup:(XXYLayerGroup *)childGroup
  ...
  for (XXYLayer *layer in reversedItems) {
    XXYAsset *asset;
    ...
    XXYLayerContainer *child = nil;
    if (asset.layerGroup) {
      // Layer is a precomp
      XXYCompositionContainer *compLayer = [[XXYCompositionContainer alloc] initWithModel:layer inLayerGroup:childGroup withLayerGroup:asset.layerGroup withAssestGroup:assetGroup];
      child = compLayer;
    } else {
      child = [[XXYLayerContainer alloc] initWithModel:layer inLayerGroup:childGroup];
    }
    if (maskedLayer) {
      maskedLayer.mask = child;
      maskedLayer = nil;
    } else {
      if (layer.matteType == XXYMatteTypeAdd) {
        maskedLayer = child;
      }
...
- (void)displayWithFrame:(NSNumber *)frame forceUpdate:(BOOL)forceUpdate {
  ...
  for (XXYLayerContainer *child in _childLayers) {
...
- (void)addSublayer:(nonnull CALayer *)subLayer
  ...
  XXYLayerContainer *child = _childMap[layerName];
  if (child) {
    if (applyTransform) {
...
- (void)setViewportBounds:(CGRect)viewportBounds {
  ...
  for (XXYLayerContainer *layer in _childLayers) {
