...
@implementation XXYRoundedRectAnimator {
  XXYPointInterpolator *_centerInterpolator;
  XXYPointInterpolator *_sizeInterpolator;
  XXYNumberInterpolator *_cornerRadiusInterpolator;
  BOOL _reversed;
}
...
- (instancetype _Nonnull)initWithInputNode:(XXYAnimatorNode *_Nullable)inputNode
  ...
  if (self) {
    _centerInterpolator = [[XXYPointInterpolator alloc] initWithKeyframes:shapeRectangle.position.keyframes];
    _sizeInterpolator = [[XXYPointInterpolator alloc] initWithKeyframes:shapeRectangle.size.keyframes];
    _cornerRadiusInterpolator = [[XXYNumberInterpolator alloc] initWithKeyframes:shapeRectangle.cornerRadius.keyframes];
...
- (void)addCorner:(CGPoint)cornerPoint withRadius:(CGFloat)radius toPath:(XXYBezierPath *)path clockwise:(BOOL)clockwise {
  ...
  if (cornerPoint.y == currentPoint.y) {
    ...
    if (cornerPoint.x < currentPoint.x) {
      ...
      [path XXY_addLineToPoint:corner];
      if (radius) {
        ...
        CGPoint cp2 = (clockwise ?
                       ...
                       CGPointMake(curvePoint.x, curvePoint.y - (radius * ellipseControlPointPercentage)));
        [path XXY_addCurveToPoint:curvePoint controlPoint1:cp1 controlPoint2:cp2];
      }
    } else {
      // Moving east
      CGPoint corner = CGPointMake(cornerPoint.x - radius, currentPoint.y);
...
- (void)performLocalUpdate {
  ...
  XXYBezierPath *path1 = [[XXYBezierPath alloc] init];
  ...
  [path1 XXY_moveToPoint:startPoint];
  if (clockWise) {
