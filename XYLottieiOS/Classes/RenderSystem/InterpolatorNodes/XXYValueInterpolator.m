...
@interface XXYValueInterpolator ()

@property (nonatomic, strong) NSArray<XXYKeyframe *> *keyframes;

@end

@implementation XXYValueInterpolator
...
- (BOOL)setValue:(id)value atFrame:(NSNumber *)frame {
  ...
  if (frame.floatValue == self.leadingKeyframe.keyframeTime.floatValue) {
    ...
    XXYKeyframe *newKeyframe = [self.leadingKeyframe copyWithData:data];
    ...
    // Is trailing frame
    XXYKeyframe *newKeyframe = [self.trailingKeyframe copyWithData:data];
    NSMutableArray *keyframes = [NSMutableArray arrayWithArray:_keyframes];
    ...
    XXYKeyframe *keyframe = [[XXYKeyframe alloc] initWithKeyframe:keyframeDict];
...
- (void)updateKeyframeSpanForFrame:(NSNumber *)frame {
  if (self.leadingKeyframe == nil &&
    ...
    XXYKeyframe *first = _keyframes.firstObject;
  ...
  if (self.trailingKeyframe && frame.floatValue >= self.trailingKeyframe.keyframeTime.floatValue) {
    ...
    XXYKeyframe *testLeading = self.trailingKeyframe;
    XXYKeyframe *testTrailing = nil;
    ...
    XXYKeyframe *testLeading = nil;
    XXYKeyframe *testTrailing = self.leadingKeyframe;
...
- (CGFloat)progressForFrame:(NSNumber *)frame {
  ...
  CGFloat progession = XXY_RemapValue(frame.floatValue, self.leadingKeyframe.keyframeTime.floatValue, self.trailingKeyframe.keyframeTime.floatValue, 0, 1);
  ...
  if ((self.leadingKeyframe.outTangent.x != self.leadingKeyframe.outTangent.y ||
      ...
      (!XXY_CGPointIsZero(self.leadingKeyframe.outTangent) &&
       !XXY_CGPointIsZero(self.trailingKeyframe.inTangent))) {
    // Bezeir Time Curve
    progession = XXY_CubicBezeirInterpolate(CGPointMake(0, 0), self.leadingKeyframe.outTangent, self.trailingKeyframe.inTangent, CGPointMake(1, 1), progession);
  }
