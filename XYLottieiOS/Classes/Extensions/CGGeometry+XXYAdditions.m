...
// XXY_RectIntegral will return {5,5}, {10, 10}.
...
CGRect XXY_RectCenteredAtPoint(CGPoint center, CGSize size, BOOL integral) {
  ...
  if (integral) { result = XXY_RectIntegral(result); }
  return result;
}
...
CATransform3D XXY_CATransform3DSlerpToTransform(CATransform3D fromXorm, CATransform3D toXform, CGFloat amount ) {
  ...
  GLKMatrix4 xform1 = XXY_GLKMatrix4FromCATransform(fromXorm);
  GLKMatrix4 xform2 = XXY_GLKMatrix4FromCATransform(toXform);
  ...
  rX.m31 = r2.y;
  rX.m32 = r2.z;
  return XXY_CATransform3DFromGLKMatrix4(rX);
}
...
CGPoint XXY_PointByLerpingPoints(CGPoint point1, CGPoint point2, CGFloat value) {
  ...
  CGPoint returnPoint;
  if (xDiff == 0 || yDiff == 0) {
    returnPoint.x = xDiff == 0 ? point1.x : XXY_RemapValue(value, 0, 1, point1.x, point2.x);
    returnPoint.y = yDiff == 0 ? point1.y : XXY_RemapValue(value, 0, 1, point1.y, point2.y);
  } else {
    CGFloat rx = transposed.x / transposed.y;
    CGFloat yLerp = XXY_RemapValue(value, 0, 1, 0, transposed.y);
    ...
    returnPoint = XXY_PointAddedToPoint(point1, interpolatedPoint);
  }
  return returnPoint;
}
...
CGFloat XXY_CubicBezierGetY(CGPoint cp1, CGPoint cp2, CGFloat T) {
...
CGPoint XXY_PointInCubicCurve(CGPoint start, CGPoint cp1, CGPoint cp2, CGPoint end, CGFloat T) {
  CGPoint A = XXY_PointInLine(start, cp1, T);
  CGPoint B = XXY_PointInLine(cp1, cp2, T);
  CGPoint C = XXY_PointInLine(cp2, end, T);
  CGPoint D = XXY_PointInLine(A, B, T);
  CGPoint E = XXY_PointInLine(B, C, T);
  CGPoint F = XXY_PointInLine(D, E, T);
  return F;
}

CGFloat XXY_SolveCubic(CGFloat a, CGFloat b, CGFloat c, CGFloat d) {
  if (a == 0) return XXY_SolveQuadratic(b, c, d);
  if (d == 0) return 0;
  
  b /= a;
  c /= a;
  d /= a;
  CGFloat q = (3.0 * c - XXY_Squared(b)) / 9.0;
  CGFloat r = (-27.0 * d + b * (9.0 * c - 2.0 * XXY_Squared(b))) / 54.0;
  CGFloat disc = XXY_Cubed(q) + XXY_Squared(r);
  CGFloat term1 = b / 3.0;
  
  if (disc > 0) {
    double s = r + sqrtf(disc);
    s = (s < 0) ? - XXY_CubicRoot(-s) : XXY_CubicRoot(s);
    double t = r - sqrtf(disc);
    t = (t < 0) ? - XXY_CubicRoot(-t) : XXY_CubicRoot(t);
    
    double result = -term1 + s + t;
    if (result >= 0 && result <= 1) return result;
  } else if (disc == 0) {
    double r13 = (r < 0) ? - XXY_CubicRoot(-r) : XXY_CubicRoot(r);
...
CGFloat XXY_SolveQuadratic(CGFloat a, CGFloat b, CGFloat c) {
  CGFloat result = (-b + sqrtf(XXY_Squared(b) - 4 * a * c)) / (2 * a);
  if (result >= 0 && result <= 1) return result;
  
  result = (-b - sqrtf(XXY_Squared(b) - 4 * a * c)) / (2 * a);
  if (result >= 0 && result <= 1) return result;
  
  return -1;
}

CGFloat XXY_Squared(CGFloat f) { return f * f; }

CGFloat XXY_Cubed(CGFloat f) { return f * f * f; }

CGFloat XXY_CubicRoot(CGFloat f) { return powf(f, 1.0 / 3.0); }
...
CGFloat XXY_CubicBezeirInterpolate(CGPoint P0, CGPoint P1, CGPoint P2, CGPoint P3, CGFloat x) {
  ...
  if (x == P0.x) {
    ...
    CGFloat d = P0.x - x;
    CGFloat tTemp = XXY_SolveCubic(a, b, c, d);
    if (tTemp == -1) return -1;
    t = tTemp;
  }
  
  // Calculate y from t
  return XXY_Cubed(1 - t) * P0.y + 3 * t * XXY_Squared(1 - t) * P1.y + 3 * XXY_Squared(t) * (1 - t) * P2.y + XXY_Cubed(t) * P3.y;
}
...
CGFloat XXY_CubicLengthWithPrecision(CGPoint fromPoint, CGPoint toPoint, CGPoint controlPoint1, CGPoint controlPoint2, CGFloat iterations) {
  ...
  for (int i = 1; i <= iterations; ++i) {
    ...
    CGPoint p = XXY_PointInCubicCurve(fromPoint, controlPoint1, controlPoint2, toPoint, s);
    
    length += XXY_PointDistanceFromPoint(previousPoint, p);
    previousPoint = p;
  }
  return length;
}
...
CGFloat XXY_CubicLength(CGPoint fromPoint, CGPoint toPoint, CGPoint controlPoint1, CGPoint controlPoint2) {
  return XXY_CubicLengthWithPrecision(fromPoint, toPoint, controlPoint1, controlPoint2, 20);
}
