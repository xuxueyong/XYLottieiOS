...
@implementation XXYAnimationView {
  XXYCompositionContainer *_compContainer;
  ...
  XXYAnimationCompletionBlock _completionBlockToRestoreWhenAttachedToWindow;
}
...
+ (nonnull instancetype)animationNamed:(nonnull NSString *)animationName inBundle:(nonnull NSBundle *)bundle {
  XXYComposition *comp = [XXYComposition animationNamed:animationName inBundle:bundle];
  return [[XXYAnimationView alloc] initWithModel:comp inBundle:bundle];
}
...
+ (nonnull instancetype)animationFromJSON:(nullable NSDictionary *)animationJSON inBundle:(nullable NSBundle *)bundle {
  XXYComposition *comp = [XXYComposition animationFromJSON:animationJSON inBundle:bundle];
  return [[XXYAnimationView alloc] initWithModel:comp inBundle:bundle];
}

+ (nonnull instancetype)animationWithFilePath:(nonnull NSString *)filePath {
  XXYComposition *comp = [XXYComposition animationWithFilePath:filePath];
  return [[XXYAnimationView alloc] initWithModel:comp inBundle:[NSBundle mainBundle]];
}
...
- (instancetype)initWithContentsOfURL:(NSURL *)url {
  ...
  if (self) {
    [self _commonInit];
    XXYComposition *laScene = [[XXYAnimationCache sharedCache] animationForKey:url.absoluteString];
    if (laScene) {
      ...
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void) {
        ...
        XXYComposition *laScene = [[XXYComposition alloc] initWithJSON:animationJSON withAssetBundle:[NSBundle mainBundle]];
        dispatch_async(dispatch_get_main_queue(), ^(void) {
          [[XXYAnimationCache sharedCache] addAnimation:laScene forKey:url.absoluteString];
          laScene.cacheKey = url.absoluteString;
          [self _initializeAnimationContainer];
          [self _setupWithSceneModel:laScene];
        });
...
- (instancetype)initWithModel:(XXYComposition *)model inBundle:(NSBundle *)bundle {
...
- (void)_setupWithSceneModel:(XXYComposition *)model {
  if (_sceneModel) {
  ...
  _sceneModel = model;
  _compContainer = [[XXYCompositionContainer alloc] initWithModel:nil inLayerGroup:nil withLayerGroup:_sceneModel.layerGroup withAssestGroup:_sceneModel.assetGroup];
...
- (void)_callCompletionIfNecessary:(BOOL)complete {
  if (self.completionBlock) {
    XXYAnimationCompletionBlock completion = self.completionBlock;
...
- (void)setSceneModel:(XXYComposition *)sceneModel {
...
- (void)playWithCompletion:(XXYAnimationCompletionBlock)completion {
  if (!_sceneModel) {
    _isAnimationPlaying = YES;
...
- (void)playToProgress:(CGFloat)progress withCompletion:(nullable XXYAnimationCompletionBlock)completion {
...
- (void)playFromProgress:(CGFloat)fromStartProgress
          ...
          withCompletion:(nullable XXYAnimationCompletionBlock)completion {
  if (!_sceneModel) {
    _isAnimationPlaying = YES;
    self.completionBlock = completion;
...
- (void)setCacheEnable:(BOOL)cacheEnable {
  ...
  if (cacheEnable) {
    [[XXYAnimationCache sharedCache] addAnimation:_sceneModel forKey:self.sceneModel.cacheKey];
  } else {
    [[XXYAnimationCache sharedCache] removeAnimationForKey:self.sceneModel.cacheKey];
  }
}
...
- (void)addSubview:(nonnull XXYView *)view
      toLayerNamed:(nonnull NSString *)layer
    applyTransform:(BOOL)applyTransform {
  [self _layout];
  CGRect viewRect = view.frame;
  XXYView *wrapperView = [[XXYView alloc] initWithFrame:viewRect];
...
- (CGFloat)animationProgress {
  if (_isAnimationPlaying &&
    ...
    CGFloat activeProgress = [self _progressForFrame:[(XXYCompositionContainer *)_compContainer.presentationLayer currentFrame]];
...
#define XXYViewContentMode UIViewContentMode
#define XXYViewContentModeScaleToFill UIViewContentModeScaleToFill
#define XXYViewContentModeScaleAspectFit UIViewContentModeScaleAspectFit
#define XXYViewContentModeScaleAspectFill UIViewContentModeScaleAspectFill
#define XXYViewContentModeRedraw UIViewContentModeRedraw
#define XXYViewContentModeCenter UIViewContentModeCenter
#define XXYViewContentModeTop UIViewContentModeTop
#define XXYViewContentModeBottom UIViewContentModeBottom
#define XXYViewContentModeLeft UIViewContentModeLeft
#define XXYViewContentModeRight UIViewContentModeRight
#define XXYViewContentModeTopLeft UIViewContentModeTopLeft
#define XXYViewContentModeTopRight UIViewContentModeTopRight
#define XXYViewContentModeBottomLeft UIViewContentModeBottomLeft
#define XXYViewContentModeBottomRight UIViewContentModeBottomRight
...
- (void)setContentMode:(XXYViewContentMode)contentMode {
...
- (void)setCompletionBlock:(XXYAnimationCompletionBlock)completionBlock {
    if (completionBlock) {
...
- (void)_layout {
  CGPoint centerPoint = XXY_RectGetCenterPoint(self.bounds);
  CATransform3D xform;

  if (self.contentMode == XXYViewContentModeScaleToFill) {
  ...
  } else if (self.contentMode == XXYViewContentModeScaleAspectFit) {
  ...
  } else if (self.contentMode == XXYViewContentModeScaleAspectFill) {
    CGFloat compAspect = self.sceneModel.compBounds.size.width / self.sceneModel.compBounds.size.height;
    CGFloat viewAspect = self.bounds.size.width / self.bounds.size.height;
    BOOL scaleWidth = compAspect < viewAspect;
    CGFloat dominantDimension = scaleWidth ? self.bounds.size.width : self.bounds.size.height;
