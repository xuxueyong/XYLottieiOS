...
#import "XXYAnimationView_Compat.h"
#import "XXYComposition.h"

typedef void (^XXYAnimationCompletionBlock)(BOOL animationFinished);

@interface XXYAnimationView : XXYView
...
/// Creates an animation from the XXYComposition, images are loaded from the specified bundle
- (nonnull instancetype)initWithModel:(nullable XXYComposition *)model inBundle:(nullable NSBundle *)bundle;
...
@property (nonatomic, copy, nullable) XXYAnimationCompletionBlock completionBlock;

/// Set the amimation data
@property (nonatomic, strong, nullable) XXYComposition *sceneModel;
 ...
 * If loopAnimation is NO the animation will stop and the comletion block will be called.
 **/
- (void)playWithCompletion:(nullable XXYAnimationCompletionBlock)completion;
 ...
 * This is helpful when adding custom subviews to a XXYAnimationView
 *
 **/
...
@property (nonatomic) XXYViewContentMode contentMode;
#endif
